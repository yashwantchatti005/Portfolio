// =====================================================
// navClick — called via onclick="navClick(event,'#id')"
// directly in HTML. This is 100% mobile-safe because
// inline onclick cannot be blocked by z-index or touch
...
(function () {
  ...
  Particle.prototype.draw = function () {
    ...
    ctx.fillStyle = this.color + this.op + ')'; ctx.fill();
    if (this.size > 1.5) {
      ctx.beginPath(); ctx.arc(this.x, this.y, this.size * 2.5, 0, Math.PI * 2);
      ctx.fillStyle = this.color + (this.op * 0.15) + ')'; ctx.fill();
    }
  };
  ...
  Star.prototype.draw = function () {
    var g = ctx.createLinearGradient(this.x, this.y, this.x - Math.cos(this.angle) * this.len, this.y - Math.sin(this.angle) * this.len);
...
// ===== TYPING EFFECT =====
(function () {
  var texts = ['Full Stack Developer Trainee','Frontend Enthusiast','Backend Explorer','Problem Solver','Space Code Explorer 🚀'];
...
function checkSkills() {
  ...
  if (sec.getBoundingClientRect().top < window.innerHeight - 80) {
    skillsDone = true;
    document.querySelectorAll('.skill-fill').forEach(function (b) { b.style.width = b.getAttribute('data-width') + '%'; });
  }
}
...
function checkCounters() {
  ...
  if (sec.getBoundingClientRect().top < window.innerHeight - 80) {
    ...
    document.querySelectorAll('.ach-value').forEach(function (el) {
      ...
      var count = 0, inc = Math.ceil(target / 60);
      function update() {
...
document.getElementById('contactForm').addEventListener('submit', function (e) {
  e.preventDefault();
  var btn = this.querySelector('button'), orig = btn.innerHTML;
...
;(function(){
  ...
  const closeBtn = document.getElementById('certImgClose');
  const imgEl   = document.getElementById('certImgEl');
  const titleEl = document.getElementById('certImgTitle');
  const orgEl   = document.getElementById('certImgOrg');
  ...
  document.querySelectorAll('.cert-card2').forEach(card => {
